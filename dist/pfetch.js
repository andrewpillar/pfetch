...
var pfetch = {
	...
	_fire: function _fire(eventName, args) {
	...
	_getContainer: function _getContainer(link) {
		var container = link.getAttribute(this._config.containerAttr);
	...
	_getTitle: function _getTitle(element) {
		var titleNode = element.getElementsByTagName("title")[0];
	...
	_handleClick: function _handleClick(e) {
		...
		fetch(href, init).then(function (response) {
			...
			if (html) {
				var container = _this._getContainer(link);
				var element = _this._createHtmlElement(html);
				...
				_this._fire("afterReplace", [fragment]);
				...
				window.history.pushState(_this._state, title, href);
			}
		});
	},

	_handlePopState: function _handlePopState(e) {
		var state = e.state;

		this._replaceContent(state.container, state.title, state.fragment);
	},

	_loadConfig: function _loadConfig(config) {
...
},{"promise-polyfill":1,"whatwg-fetch":2}]},{},[3]);
